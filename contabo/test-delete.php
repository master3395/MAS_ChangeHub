...
if (count($snapshots) > 0) {
    $oldestSnapshot = $snapshots[0]; // First one is oldest after sorting
    $snapshotId = $oldestSnapshot['snapshotId'];
    $snapshotName = $oldestSnapshot['name'];
    
    echo "Attempting to delete oldest snapshot:\n";
    echo "  ID: $snapshotId\n";
    echo "  Name: $snapshotName\n\n";
    
    // Test different delete endpoints
    $endpoints = [
        "/compute/snapshots/$snapshotId",
        "/compute/instances/202441688/snapshots/$snapshotId",
        "/snapshots/$snapshotId"
    ];
    
    foreach ($endpoints as $endpoint) {
        echo "Testing DELETE endpoint: $endpoint\n";
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POSTFIELDS => json_encode(['confirm' => true]),
            CURLOPT_HTTPHEADER => [
                'Authorization: Bearer ' . $accessToken,
                'Content-Type: application/json',
                'x-request-id: ' . sprintf('%04x%04x-%04x-%04x-%04x-%04x%04x%04x',
